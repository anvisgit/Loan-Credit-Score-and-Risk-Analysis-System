"""
app/db_connect.py — PostgreSQL connection + all DB helper functions.
...
def execute(sql: str, params=None):
    """Execute INSERT/UPDATE/DELETE. Returns lastrowid if available."""
    conn = get_connection()
    try:
        with conn:
...
def get_customer_credit_scores(customer_id: int) -> pd.DataFrame:
    return fetch_df("""
...
def get_latest_credit_score(customer_id: int) -> Optional[dict]:
    df = fetch_df("""
...
def insert_customer(name, dob, phone, email,
                    monthly_income, employment_type, company_name,
                    street, city, zip_code) -> int:
    customer_id = execute("""
        INSERT INTO customer (name, dob, phone, email)
        VALUES (%s, %s, %s, %s)
        RETURNING customer_id
    """, (name, dob, phone, email))

    execute("""
        INSERT INTO income_details (customer_id, monthly_income, employment_type, company_name)
        VALUES (%s, %s, %s, %s)
    """, (customer_id, monthly_income, employment_type, company_name))

    execute("""
        INSERT INTO address_details (customer_id, street, city, zip_code)
        VALUES (%s, %s, %s, %s)
    """, (customer_id, street, city, zip_code))

    execute("""
        INSERT INTO credit_history (customer_id, total_loans, total_defaults)
        VALUES (%s, 0, 0)
    """, (customer_id,))
...
def update_customer(customer_id, name, phone, email, dob):
    execute("""
        UPDATE customer SET name=%s, phone=%s, email=%s, dob=%s
        WHERE customer_id=%s
...
def insert_credit_score(customer_id: int, score_value: int):
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT risk_id FROM risk_category
                ...
                cur.execute("""
                    INSERT INTO credit_score (customer_id, score_value, score_date, risk_id)
...
def insert_loan_type(type_name, base_interest_rate, max_amount):
    execute("""
        INSERT INTO loan_type (type_name, base_interest_rate, max_amount)
        VALUES (%s, %s, %s)
...
def update_loan_type(loan_type_id, type_name, base_interest_rate, max_amount):
    execute("""
        UPDATE loan_type
        SET type_name=%s, base_interest_rate=%s, max_amount=%s
...
def insert_risk_category(risk_level, min_score, max_score):
    execute("""
        INSERT INTO risk_category (risk_level, min_score, max_score)
        VALUES (%s, %s, %s)
...
def update_risk_category(risk_id, risk_level, min_score, max_score):
    execute("""
        UPDATE risk_category SET risk_level=%s, min_score=%s, max_score=%s
        WHERE risk_id=%s
...
def get_loans_for_customer(customer_id: int) -> pd.DataFrame:
    return fetch_df("""
...
def get_all_loans() -> pd.DataFrame:
    return fetch_df("""
        SELECT l.loan_id, c.name AS customer_name, lt.type_name,
               ...
               l.current_status, l.disbursed_date
        FROM   loan l
...
def insert_loan(customer_id, loan_type_id, loan_amount,
                interest_rate, tenure, status="Active") -> int:
    loan_id = execute("""
        INSERT INTO loan (customer_id, loan_type_id, loan_amount,
                          interest_rate, tenure, current_status, disbursed_date)
    ...
    # Status history
    execute("""
        INSERT INTO loan_status_history (loan_id, status_label, change_date)
        VALUES (%s, 'Approved', CURRENT_DATE)
    """, (loan_id,))
    execute("""
        INSERT INTO loan_status_history (loan_id, status_label, change_date)
    ...
    execute("""
        UPDATE credit_history SET total_loans = total_loans + 1, last_updated = CURRENT_DATE
        WHERE customer_id = %s
    """, (customer_id,))
...
def generate_emi_schedule(loan_id: int, loan_amount: float,
                           interest_rate: float, tenure: int):
    """Generate and insert EMI schedule rows for a loan."""
    r = interest_rate / 1200.0
    if r == 0:
        emi = round(loan_amount / tenure, 2)
    else:
        emi = round(
            loan_amount * r * (1 + r) ** tenure /
            ((1 + r) ** tenure - 1), 2
        )
...
def get_overdue_emis(loan_id: int) -> pd.DataFrame:
    return fetch_df("""
        SELECT emi_id, due_date, emi_amount
        FROM   emi_schedule
        WHERE  loan_id = %s
          AND  status = 'Overdue'
        ORDER BY due_date
...
def record_payment(emi_id: int, paid_amount: float, payment_mode: str):
    """Record repayment and update EMI status."""
    execute("""
    ...
    execute("""
        UPDATE emi_schedule SET status = 'Paid' WHERE emi_id = %s
    """, (emi_id,))
...
def auto_default_overdue(loan_id: int):
    """If any EMI is overdue, insert a default_record and penalty."""
    ...
    existing = fetch_df(
        "SELECT default_id FROM default_record WHERE loan_id=%s", (loan_id,)
    )
    ...
    today = date.today()
    overdue_days = int((today - pd.to_datetime(df["due_date"].min()).date()).days)

    default_id = execute("""
        INSERT INTO default_record (loan_id, default_date, overdue_days)
        VALUES (%s, CURRENT_DATE, %s)
        RETURNING default_id
    """, (loan_id, overdue_days))

    # Penalty = 2% of overdue EMI total per 30 days
    total_overdue = float(df["emi_amount"].sum())
    penalty_amount = round(total_overdue * 0.02 * max(1, overdue_days // 30), 2)

    execute("""
        INSERT INTO penalty (default_id, penalty_amount, penalty_date)
        VALUES (%s, %s, CURRENT_DATE)
...
def get_kpis() -> dict:
    ...
    df_defaults  = fetch_df("SELECT COUNT(DISTINCT loan_id) AS cnt FROM default_record")
    df_penalties = fetch_df("SELECT COALESCE(SUM(penalty_amount),0) AS total FROM penalty")

    return {
        "total_customers": int(df_customers["cnt"].iloc[0]),
        "active_loans":    int(df_loans["cnt"].iloc[0]),
        "defaulters":      int(df_defaults["cnt"].iloc[0]),
        "penalties_collected": float(df_penalties["total"].iloc[0]),
    }


def get_loans_by_risk() -> pd.DataFrame:
    return fetch_df("""
        SELECT
            COALESCE(rc.risk_level, 'Unknown') AS risk_level,
...
def get_monthly_repayments() -> pd.DataFrame:
    return fetch_df("""
        SELECT
            TO_CHAR(payment_date, 'YYYY-MM') AS month,
            SUM(paid_amount)                 AS total_collected,
            COUNT(repayment_id)              AS payment_count
        FROM repayment
        GROUP BY month
        ORDER BY month
    """)
...
def get_defaulters_table() -> pd.DataFrame:
    return fetch_df("""
        SELECT
            c.customer_id, c.name, c.phone,
            l.loan_id, lt.type_name AS loan_type, l.loan_amount,
            dr.default_date, dr.overdue_days,
...
def export_table_csv(table_name: str) -> pd.DataFrame:
    allowed = {
        "customer", "income_details", "address_details", "credit_history",
        "credit_score", "loan", "loan_type", "emi_schedule", "repayment",
        "default_record", "penalty", "guarantor", "risk_category",
        "loan_status_history"
    }
    if table_name not in allowed:
        raise ValueError(f"Table '{table_name}' not allowed for export.")
